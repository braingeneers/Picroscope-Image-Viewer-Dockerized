...
run:
	docker run -it -p 8080:8080 --rm --name $(IMAGE_NAME) $(CONTAINER)

run-prod:  	# run in production on gi server
	docker run -d --rm -p 127.0.0.1:8055:8080 --name $(IMAGE_NAME) $(CONTAINER)
...
run-dev:	# run local dev instance in docker container
	docker run -d --rm -p 0.0.0.0:8055:8080 --name $(IMAGE_NAME) $(CONTAINER)
